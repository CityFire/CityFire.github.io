...
- 把重复性的编码工作交给`ChatGPT`或`Marscode`等AI助手，自己专注在更有价值的设计工作上
- 用AI工具辅助进行性能分析，代码框架，避免写出臃肿代码
...
- 有了新想法，可以直接用`Coze`等平台快速做出一个简单demo，用实际效果来验证想法
- 在跟技术团队沟通时，能更好地理解技术可行性，设计出更实用的功能
- 产品原型不再局限于静态界面，可以直接用`ChatGPT`或`Marscode`等AI工具生成可交互的演示版本
...
- 带你初步了解`Marscode`这个开发效率神器
- 帮你掌握AI辅助开发的技巧

### 学习AI编程的平台

在小白入⻔AI编程的时候，一上来就接触AI编程工具并不是一个很好的选择。

市面上的AI编程工具:例如国内字节的MarsCode、阿里的通义灵码或者国外的Cursor

它们的目的是帮助已经学会编程的同学提高开发效率，但对于对编程基础概念都还不是很清楚的同学，这些 工具并不友好

我们将AI编程学习分为两个阶段，并给大家推荐下适合小白入⻔和进阶的学习平台和工具
...
在搭建智能体工作流的过程中，你会使用插件节点、数据库节点、变量节点等，这些节点都蕴含着编程的概 念，但是又不需要你编码。在实践的过程中就将这些概念掌握了
...
这里不过多介绍，如果你还没有接触过任何一⻔编程语言，建议你直接选择Python
...
这个全称概括了IDE的核心特点 - 它是一个将多种编程工具和功能集成在一起的软件开发环境，旨在提高程 序员的工作效率和便利性。
...
首先我们需要了解一门语言的基础语法，这里我推荐Python：比如| B站3小时Python代码基础

PS：编程语言了解一种即可，各种语言都是相通的，而且以后就算有了更高級的语言，只会学习起来更简单
...
1.存储数据：将数据存储在变量中，使数据有一个名字标识，可以顾时访问和修改。
...
```
Python
1 # 创建一个名叫"age"的储物盒，放入数字18
2 age = 18
3
4 # 创建一个名叫"name"的储物盒，放入文字"小明" 5 name = "小明"
6
7 # 可以随时查看盒子里的内容
8 print(f"{name}今年{age}岁") # 输出:小明今年18岁 
9
10 # 可以随时更换盒子里的内容
11 age = 19 # 把age盒子里的18换成19
12 print(f"{name}明年{age}岁") # 输出:小明明年19岁
```
...
```
1 def cook_dish(ingredients):     # 输入:⻝材
2    # 处理:烹饪步骤
3    if not check_ingredients(ingredients):  # 检查⻝材是否⻬全
4         return "⻝材不足"
5
6    prepared = prepare_ingredients(ingredients)  # 处理⻝材
7    cooked = cooking_process(prepared)           # 烹饪过程
8
9    return cooked # 输出:完成的菜品
...
2. 下一个工作台(函数B)拿到周转箱里的半成品，根据产品类型(if条件)选择不同的加工方式，有时需 要重复加工几次(循环)

3. 就这样一个工作台接着一个工作台，最终制造出成品 我们用AI写公众号文章的例子来理解这个流程:

```
Python
1 def generate_article(hotNews):
2
3 # 第一个函数:选题
4 topic = generate_topic(hotNews) # 输入热点新闻，输出选题
5
6 # 第二个函数:生成大纲
7 outline = generate_outline(topic) # 输入topic，输出outline
8
9 # 第三个函数:根据大纲生成内容
10 content = generate_content(outline) # 输入outline，输出content
11
12 # 第四个函数:生成摘要
13 summary = generate_summary(content) # 输入content，输出summary
14
15 # 第五个函数:生成封面
16 post = generate_post(summary) # 输入summary，输出post
17
18 # 最终的文章包含这些部分
19  article = {
20     "topic": topic,
...
我们应该知道ChatGPT刚火的时候，国内也出现了好多的平台号称可以付费使用和ChatGPT的能力，这个就 是我们说的套壳软件，

这些平台肯定没有自己的大模型，他们只是接入了ChatGPT的API接口而已，甚至接入的都不一定是 ChatGPT的能力

还有各种你在微信上⻅到的各种小程序，大多数的底层都是接入了一些开源或者付费的API，然后进行简单的前端⻚面包装就成了一个新产品了

所以如果你对API有一定的认知和了解，理论上来说你就可以通过像搭积木一样配合各种API组装出你的产 品。现在好多的创业公司也是这么做的
...
**PS:新人接触数据库的时候就可以类比Excel，或者微信、⻜书的多维表格**
...
最常⻅的就是云服务器，比如阿里云、腾讯云提供的服务器。它们就像是租用的一台远程电脑，你可以把你的代码放到上面运行。
...
上面我们花费大篇幅讲解了编程的基础概念，目的是帮助大家快速入⻔。
...
- `Marscode`、`Cursor`这样的AI编程IDE，就像是给程序员配备了一个AI助手
- 它们可以：
  - 帮你生成代码
  - 解释代码含义
  - 优化代码结构
...
想要创造一个简单的对话机器人，也许`Coze`就够用了。
需要开发一个复杂的企业应用？那你需要借助`Marscode`这样的专业工具。
...
2. 对于阅读代码的同学来讲，可以通过添加注释防止重复的理解的代码
...
旁边多了一个可以随时问答的AI助手，而Marscode的点在于这个大模型是专⻔针对代码进行优化过的
...
3. 当产品和交互就绪之后，流程就进入到开发手中，也就是我们常说的程序员
```
  常见的开发角色有两个：前端和后端开发
