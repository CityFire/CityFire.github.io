...
- 代码组织混乱，模块间的耦合度高，导致一个模块的变动可能对其他模块产生影响，从而使代码难以维护。
...
本文将从`逻辑模型`、`开发模型`和`部署模型`分别介绍应用分层架构设计规则。

## 逻辑模型

**图1** 分层架构逻辑模型

![图1](images/hierarchicalArchitecture.png)
...
## 开发模型
**图2** 分层架构开发模型
![图2](images/hierachArchitectureDev.png)

- **产品定制层**
产品定制层的各个子目录会被编译成一个`Entry类型的HAP`，作为应用的主入口。该层主要针对跨多种设备，为各种设备形态集成相应的功能和特性。产品定制层被划分为多个功能模块，每个功能模块都针对特定的设备或使用场景设计，并根据具体的产品需求进行功能及交互的定制开发。
...
## 部署模型
**图3** 分层架构部署模型（不同设备的定制）
![图3](images/hierachArchitectureDeploy.png)
